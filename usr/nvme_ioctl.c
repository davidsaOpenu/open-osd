	    ...
	    void *metadata, __u16 metadata_len)
{
	struct nvme_user_io io = {
		...
		.control	= control,
		.nblocks	= nblocks,
		.rsvd		= metadata_len, /* since there's no other field that can be used to pass the metadata buffer length, we've decided to use the reserved field; this is a contract between the device driver and our nvme-cli. */
		.metadata	= (__u64)(uintptr_t) metadata,
		.addr		= (__u64)(uintptr_t) data,
		.slba		= slba,
		.dsmgmt		= dsmgmt,
